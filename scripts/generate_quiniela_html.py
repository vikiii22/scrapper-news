...
def load_prediction_data() -> list[dict]:
    """Carga predicciones desde Mongo y, si no está disponible, desde JSON procesado."""
    try:
...
def generate_static_html(output_path: Path):
    ...
    """
    # Cargar datos
    quiniela_data = load_prediction_data()
    if not quiniela_data:
        raise ValueError("No se encontraron predicciones ni en MongoDB ni en JSON procesado.")
